...
The publisher also has a recovery layer for short YOLO tracking drops near the doorway. `YOLO_TRACKER` defaults to `backend/publisher_yolo/botsort_stable.yaml`, which keeps lost tracks alive longer and requires stronger evidence before creating a new id. `TRACK_LOST_GRACE_FRAMES` controls how long a missing person can be recovered, `TRACK_MATCH_MAX_DISTANCE_PIXELS` controls how close a new track must be to the lost one, and `TRACK_EVENT_SUPPRESSION_FRAMES` prevents duplicate events from one physical crossing after an id swap.
